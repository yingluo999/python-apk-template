...
class MainApp(App):
    def build(self):
        ...
        title = Label(text="🐉 十二生肖注册机器人", font_size=22, size_hint_y=None, height=50,
                      color=(0.8, 0.1, 0.2, 1), bold=True)
        ...
        self.count_input = TextInput(text='1', multiline=False, font_size=16, size_hint_x=0.3,
                                     background_color=(1, 1, 1, 1), foreground_color=(0.1, 0.1, 0.1, 1),
                                     input_filter='int')
    ...
    def update_log(self, dt):
        if self.log_buffer:
            display_lines = self.log_buffer[-200:]
    ...
    def run_register(self, ref_code, count):
        ...
        def progress_callback(current, total):
            if total > 0:
                Clock.schedule_once(lambda dt: setattr(self.progress, 'value', (current/total)*100), 0)
                self.status_label.text = f"⏳ 正在注册: {current}/{total}"
